...
//   console.log('Pesquisa Categoria >>>', await getCategories('ux'));
...
//   console.log("Associa usuário a categoria >>>", await associateUserToCategory(10, 4))
...
async function associateUserToCategory(userId, categoryId) {
  const config = setFetchConfig(HTTP_METHODS.PATCH);

  return await fetch(
    `${API_URL}/users/${userId}/associate/${categoryId}`,
    config
  )
    .then((response) => response.json)
    .then((data) => data)
    .catch((err) => console.warn("Error to update user: ", err));
}
